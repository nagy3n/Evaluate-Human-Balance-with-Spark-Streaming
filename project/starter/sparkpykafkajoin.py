...
redisServerStreamingDF.withColumn("value", from_json("value", redisMessageSchema)).select(col("value.*"))\
...
zSetEntriesEncodedStreamingDF = spark.sql("select zSetEntries[0].element as customer from RedisSortedSet")
...
stediEventsStreamingDF.withColumn("value", from_json("value", eventRiskCustomerSchema)).select(col("value.*"))\
...
customerRiskWithBirthYearDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("customer = email"))
...
customerRiskWithBirthYearDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value")\
    ...
    .format("kafka")\
    .option("kafka.bootstrap.servers", "kafka:19092")\
    .option("topic", "customer-with-score")\
